...
class SpectrogramDifferenceProcessor(BufferProcessor):
    ...
    def process(self, data, **kwargs):
        ...
        #        sample rate changes from song to song! The buffer_size should
        #        change then as well...
        if self.diff_frames is None:
            ...
            # the buffer might be initialised with buffer_size=None
            self.buffer_size = self.diff_frames
        # init the buffer
        # Note: do this here and not in BufferProcessor since it has to be
        #       done in a very special way (circular padding)
        if self.buffer is None:
            # TODO: refactor to super(..., self).init(buffer_size)?
            self.buffer = np.repeat(data[:1] * 0, self.buffer_size + len(data),
                                    axis=0)
            # we need to fill the last buffer_size with the data this will be
            # put at the beginning if we call the buffer
            if len(data) >= self.buffer_size:
                self.buffer[-self.buffer_size:] = data[:self.buffer_size]
            else:
